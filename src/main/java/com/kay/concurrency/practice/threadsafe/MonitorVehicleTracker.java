 ...
 * <p>
 * 1. unmodifiableMap 包装只会限定被包装的 Map 不能添加删除元素，但是其中的对象是可以改变的 2. 创建新的 map 添加新的构造的对象加入到 map 中，这样对 map
 * 元素对象的修改就不会改变原有对象
 * <p>
 * 缺点： 1. 如果 locations 集合比较大，会有性能问题，因为 DeepCopy 是在一个同步快中进行的 2. 对 locations
 * 的更新并不会反映到返回的结果对象中去，返回的只是一个快照
 */
@ThreadSafe
