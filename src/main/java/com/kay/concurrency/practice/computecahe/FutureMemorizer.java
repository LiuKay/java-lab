...
/**
 * V 是 future而不是一个确定的 result 的话就会带来另外的缓存污染问题： 1 如果某次计算任务失败或取消，那么下次同样的任务还是会失败或取消
 *
 * @param <A>
 * @param <V>
 */
		...
		public V compute(A arg) throws InterruptedException {
				...
				while (true) {
						...
						if (future == null) {
								...
								future = cache.putIfAbsent(arg,
										futureTask); // 这是一个原子操作，如果不存在指定 Key 则返回null,后续再计算，当然，用 computeIfAbsent 更直观
								if (future == null) {
										future = futureTask;
										futureTask.run();
								}
